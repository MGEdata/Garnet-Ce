...
dataset = pd.read_csv("T60_training set.csv")
x = dataset.iloc[:, 0:6]
y = dataset.iloc[:, 6]
ss_x = StandardScaler()
...
def cal_ei(mydata, searchspace0):
    ego = (searchspace0[:, 0] - max(mydata[:, -1]))/(searchspace0[:, 1])
    ei_ego = searchspace0[:, 1]*ego*stats.norm.cdf(ego) + searchspace0[:, 1]*stats.norm.pdf(ego)
    return (ei_ego)


dataset1 = pd.read_csv("T60_test set.csv")
X1 = dataset1.iloc[:, 0:6]
...
EI = cal_ei(dataset.iloc[:, 6].values.reshape(-1, 1), pre_all)
aEI1 = np.array(EI)
np.savetxt('1.1.txt', aEI1)
pre_all = np.column_stack((pre_all, EI))


data_write(pre_all, "222.xlsx")
